...
class ETPlugin_BBCode extends ETPlugin
{
	...
	public function handler_conversationController_getEditControls($sender, &$controls, $id)
	{
		...
		addToArrayString($controls, "strike", "<a href='javascript:BBCode.strike(\"$id\");void(0)' title='".T("Strike")."' class='bbcode-s'><span>".T("Strike")."</span></a>", 0);
	...
	public function handler_format_beforeFormat($sender)
	{
		...
		{
			$sender->content = preg_replace($regexp, "'$1' . \$hideFixed(\$this->blockFixedContents, '$2')", $sender->content);
		}
