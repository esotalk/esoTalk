...
class ETPlugin_Debug extends ETPlugin
{
	...
	function handler_pageEnd($sender)
	{
		...
		echo "<div id='debug'><div id='debugHdr'><h2>".sprintf(T("Page loaded in %s seconds"), $time)."</h2></div>";

		// Include the geshi library so we can syntax-highlight MySQL queries.
		include "geshi/geshi.php";

		echo "<h3><a href='#' onclick='$(\"#debugQueries\").slideToggle(\"fast\");return false'>".T("MySQL queries")." (<span id='debugQueriesCount'>".count($this->queries)."</span>)</a></h3><div id='debugQueries' class='section'>";
		...
		echo "</div>
			...
			<div id='debugPostGetFiles' class='section'><p style='white-space:pre' class='fixed' id='debugPost'>\$_POST = ";
	...
	<div id='debugSessionCookie' class='section'><p style='white-space:pre' class='fixed' id='debugSession'>\$_SESSION = ";
		...
		echo "<script>$(function() { $('#debug .section').hide(); });</script>";
	}
