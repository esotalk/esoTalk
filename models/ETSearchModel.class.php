...
class ETSearchModel extends ETModel
{
	...
	public function getConversationIDs($channelIDs = array(), $searchString = "", $orderBySticky = false)
	{
		...
		if (!$this->limit)
		{
			$this->limit = C("esoTalk.search.results") + 1;
		}
	...
	public function gambitActive(&$search, $term, $negate)
	{
		...
		{
			case T("gambit.minute"): $search->matches["b"] *= 60;
				break;
			case T("gambit.hour"): $search->matches["b"] *= 3600;
				break;
			case T("gambit.day"): $search->matches["b"] *= 86400;
				break;
			case T("gambit.week"): $search->matches["b"] *= 604800;
				break;
			case T("gambit.month"): $search->matches["b"] *= 2626560;
				break;
	...
	public static function gambitMoreResults(&$search, $term, $negate)
	{
		if (!$negate)
		{
			$search->limit(C("esoTalk.search.moreResults"));
		}
	}
