...
/**
 * The member model provides functions for retrieving and managing member data. 
 * It also provides methods to handle "last action" types.
 *
 * @package   esoTalk
...
class ETMemberModel extends ETModel {
	...
	/**
	 * @var array Reserved user names which cannot be used.
	 */
	...
	/**
	 * @var array An array of last action types => their callback functions.
	 **/
	...
	{
		// Make the groups into an array of groupId => names. 
		// Possibly consider using ETGroupModel::getAll()
		...
		if (array_key_exists("groups", $member) and array_key_exists("groupNames", $member)) {
			$member["groups"] = array_combine(
				explode(",", 
				$member["groups"]),
				explode(",", $member["groupNames"])
			);
		}
	...
	public function validateUsername($username, $checkForDuplicate = true)
	{
		...
		if ($checkForDuplicate and $cnt_rows > 0) {
			return 'nameTaken';
		}
	}
	...
	{
		// The user must be an administrator, 
		// and the root admin's permissions can't be changed. 
		// A user also cannot change their own permissions.
		return (
			ET::$session->isAdmin() 
			and  ! in_array($member['memberId'], array(C('esoTalk.rootAdmin'), ET::$session->userId))
		);
	}
	...
	{
		// The user must be an administrator, or they must have the "canSuspend" permission 
		// and the member's account be either "member" or "suspended". A user cannot suspend 
		// or unsuspend themselves, and the root admin cannot be suspended.
		return (
			(
				ET::$session->isAdmin() 
				or (
					ET::$session->user['canSuspend'] 
					and in_array($member['account'], array(ACCOUNT_MEMBER, ACCOUNT_SUSPENDED))
				)
			) 
			and ! in_array($member['memberId'], array(C('esoTalk.rootAdmin'), ET::$session->userId))
		);
	...
	public function setGroups($member, $account, $groups = array())
	{
		...
		// Now we need to create a new activity item, 
		// and to do that we need the names of the member's groups.
		...
		ET::activityModel()->create('groupChange', $member, ET::$session->user, $data);
	...
	{
		// Merge the member's old preferences with the new ones, 
		// giving preference to the new ones. Geddit?!
	...
	public function updateLastAction($type, $data = array())
	{
		...
		return $this->updateById(
			ET::$session->userId, 
			array('lastActionTime' => time(), 'lastActionDetail' => serialize($data))
		);
	}
	 ...
	 * @param mixed $callback The callback function that will be called to format the last action for display. 
	...
	/**
	 * Get formatted last action info for a member,
	 * given their lastActionTime and lastActionDetail fields.
	 *
	...
	{
		// If there is no action, or the time passed since 
		// the user was last seen is too great, then return no info.
...
ETMemberModel::addLastActionType(
	'viewingConversation', 
	array('ETMemberModel', 'lastActionViewingConversation')
);
ETMemberModel::addLastActionType(
	'startingConversation', 
	array('ETMemberModel', 'lastActionStartingConversation')
);
