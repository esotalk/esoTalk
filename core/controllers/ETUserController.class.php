...
class ETUserController extends ETController {
	...
	/**
	 * A message to display on the login form. This is useful to set in 
	 * the ETController::render404 method where we create a user controller
	...
	public function login()
	{
		...
		if ($form->validPostBack('username')) {
			...
			if (ET::$session->login($username, $form->getValue('password'), $form->getValue('remember'))) {
				...
				if (in_array('emailNotYetConfirmed', $errors)) {
					$this->renderMessage(
						'Error', 
						sprintf(T('message.emailNotYetConfirmed'), URL('user/sendConfirmation/'.$username))
					);
					return;
				} elseif (in_array('accountNotYetApproved', $errors)) {
	...
	{
		$subject = sprintf(T('email.confirmEmail.subject'), $username);

		$message = sprintf(T('email.header'), $username);
		$message .= sprintf(
			T('email.confirmEmail.body'), 
	...
	public function confirm($hash = '')
	{
		...
		// See if there is an unconfirmed user with this ID and password hash. 
		// If there is, confirm them and log them in.
		$result = ET::SQL()
			->select('1')
			->from('member')
		...
		if ($result->numRows()) {
			...
			ET::memberModel()->updateById(
				$memberId, 
				array('resetPassword' => null, 'confirmed' => true)
			);
	...
	public function sendConfirmation($username = '')
	{
		...
		$member = ET::memberModel()->get(array('m.username' => $username, 'confirmedEmail' => false));
		$member = reset($member);
	...
	/**
	 * Show the forgot password sheet, allowing a member to be sent 
	 * an email containing a link to reset their password.
	 *
	 * @return void
	 */
	public function forgot()
	{
		...
		if ($form->validPostBack('submit')) {
			...
			if ( ! $member) {
				...
				$subject = sprintf(T("email.forgotPassword.subject"), $member["username"]);

				$message = sprintf(T('email.header'), $member['username']);
				$message .= sprintf(
					T('email.forgotPassword.body'),
					C('esoTalk.forumTitle'), 
					URL("user/reset/".$member['memberId'].$hash, true)
				);
	...
	/**
	 * Show a form allowing the user to reset their password, 
	 * following on from a link sent to them by the forgot password process.
	 * 
	...
	public function reset($hashString = '')
	{
		...
		// Find the member with this password reset token. 
		// If it's an invalid token, take them back to the email form.
		$member = ET::memberModel()->get(array('m.memberId' => $memberId, 'resetPassword' => md5($hash)));
		$member = reset($member);
		...
		if ($form->validPostBack('submit')) {
			...
			if ( ! $form->errorCount()) {
				...
				$model = ET::memberModel();
				$model->updateById($memberId, $password);
