...
class ETMemberController extends ETController
{
	...
	protected function profile($member, $pane = "")
	{
		...
		if ($model->canSuspend($member))
		{
			...
			{
				$controls->add("removeAvatar", "<a href='".URL("member/removeAvatar/".$member["memberId"]."?token=".ET::$session->token)."' id='removeAvatarLink'>".T("Remove avatar")."</a>");
			}
			$controls->separator();
		}
		...
		elseif (ET::$session->userId)
		{
			...
			$actions->add("privateStart", "<a href='".URL("conversation/start/".urlencode($member["username"]))."'>".sprintf(T("Start a private conversation with %s"), $member["username"])."</a>");
		}
