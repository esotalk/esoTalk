...
class ETConversationController extends ETController
{
	...
	public function index($conversationId = false, $year = false, $month = false)
	{
		...
		{
			$startFrom = min($startFrom, $conversation["countPosts"] - 1);
		}
		...
		if ($this->responseType === RESPONSE_TYPE_DEFAULT)
		{
			...
			if (ET::$session->user)
			{
				$this->addJSLanguage("Starred", "Unstarred", "message.confirmLeave", "message.confirmDiscardReply", "Mute conversation", "Unmute conversation");
			}
			...
			$this->addJSVar("conversation", array(
				...
				"lastRead" => (ET::$session->user and $conversation["conversationId"]) ? (int) max(0, min($conversation["countPosts"], $conversation["lastRead"])) : (int) $conversation["countPosts"],
	...
	public function start($member = false)
	{
		...
		{
			ET::$session->store("channelId", reset(array_keys($channels)));
		}
