		...
		if ($handle = opendir(PATH_PLUGINS))
		{
			while (false !== ($file = readdir($handle)))
			{
				...
				if ($file[0] != "." and file_exists($pluginFile = PATH_PLUGINS."/$file/plugin.php") and (include_once $pluginFile))
				{
					...
					{
						$plugins[$file]["settingsView"] = ET::$plugins[$file]->settings($this);
					}
				}
			}
			closedir($handle);
...
// Install an uploaded plugin.
	protected function installPlugin()
	{
		...
		if (!($files = unzip("plugins/{$_FILES["installPlugin"]["name"]}", "plugins/")))
		{
			$this->esoTalk->message("invalidPlugin");
		}
		else
		{
			...
			foreach ($files as $k => $file)
			{
				...
				if ($file["directory"] and substr_count($file["name"], "/") < 2)
				{
					$directories++;
				}

				// Make sure there's an actual plugin file in there.
				if (substr($file["name"], -10) == "plugin.php")
				{
					$pluginFound = true;
				}
			}
			...
			if ($pluginFound and $directories == 1)
			{
				...
				foreach ($files as $k => $file)
				{
					...
					if ($file["directory"] and !is_dir("plugins/{$file["name"]}"))
					{
						mkdir("plugins/{$file["name"]}");
					}
				...
				if (!$error)
				{
					$this->esoTalk->message("pluginAdded");
				}
			}
