...
class ETController extends ETPluggable
{
	...
	public function getViewPath($view)
	{
		...
		if (file_exists($skinView = ET::$skin->getView($view)))
		{
			return $skinView;
		}
		...
		{
			if (file_exists($pluginView = $v->getView($view)))
			{
				return $pluginView;
			}
		}
	...
	protected function aggregateFiles($files, $type)
	{
		...
		if (!file_exists($file) or filemtime($file) < $lastModTime)
		{
			...
			foreach ($files as $f)
			{
				...
				if ($type == "css")
				{
					$content = str_replace("url(", "url(".getResource(pathinfo($f, PATHINFO_DIRNAME)."/"), $content);
				}
				$contents .= $content." ";
			}
